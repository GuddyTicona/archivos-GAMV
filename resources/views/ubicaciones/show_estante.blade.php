                                ...
                                @forelse($ubic->financieras as $item)
                                    <div class="d-flex justify-content-center align-items-center flex-wrap gap-2">
                                        ...
                                        <form action="{{ route('ubicaciones.destroy', $ubic->id) }}" method="POST"
                                            class="d-inline" onsubmit="return confirm('¿Seguro que deseas eliminar esta ubicación?');">
                                            @csrf
                                            @method('DELETE')
...
@foreach($ubicaciones as $ubic)
    @foreach($ubic->financieras as $item)
        ...
        <div class="modal fade" id="modalArchivo{{ $item->id }}" tabindex="-1" aria-labelledby="modalArchivoLabel{{ $item->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title" id="modalArchivoLabel{{ $item->id }}">Detalle del Archivo — {{ $item->codigo }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form>
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" value="{{ $item->unidad->nombre_unidad ?? 'N/D' }}" disabled>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control"
                                        value="Estante {{ $ubic->estante }} — Fila {{ $ubic->fila }} — Columna {{ $ubic->columna }}" disabled>
                                </div>
                            </div>
