...
#chat-widget img { width: 65px; height: 65px; border-radius: 50%; object-fit: cover; pointer-events: none; }
...
#chat-send:hover { background: rgb(3, 78, 95); }
</style>
</head>
<body>
...
// Ruta hacia tu controlador
const assistantMessageUrl = "{{ url('/chatbot') }}";
...
function openChat() {
    ...
    setTimeout(() => { chatBox.style.transition='opacity 0.3s, transform 0.3s'; chatBox.style.opacity=1; chatBox.style.transform='scale(1)'; },10);
    chatInput.focus();
}
function closeChat() {
    chatBox.style.transition='opacity 0.2s, transform 0.2s';
    chatBox.style.opacity=0;
    chatBox.style.transform='scale(0.9)';
    setTimeout(()=>{chatBox.style.display='none';},200);
}
chatWidget.addEventListener('click',()=>{ chatBox.style.display==='flex'?closeChat():openChat(); });
...
function addMessage(text,sender){
    ...
    if(sender==='bot'){ p.innerHTML=text; } else { p.textContent='Tú: '+text; }
...
function sendMessage(){
    ...
    .catch(()=>{ chatContent.removeChild(typingDiv); addMessage('Error al procesar el mensaje.','bot'); })
